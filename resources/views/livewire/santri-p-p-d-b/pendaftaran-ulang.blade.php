            ...
            @if($santri->status_pembayaran === 'verified')
                <div class="bg-white rounded-lg shadow-lg p-8 text-center">
                    ...
                    <p class="text-gray-600 mb-6">
                        Selamat! Pembayaran Anda telah diverifikasi dan pendaftaran ulang telah selesai.
                    </p>
                    <a href="{{ route('check-status') }}" class="inline-block px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition duration-300">
        ...
        <div class="bg-white rounded-xl card-shadow p-6 mb-8">
                ...
                <form wire:submit.prevent="submit" id="registrationForm" class="space-y-8">
                    ...
                    <div id="step1" class="{{ $formPage == 1 ? 'block' : 'hidden' }} bg-white rounded-lg shadow-lg p-8">
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" wire:model="pembayaranForm.nominal_pembayaran" 
                            ...
                            <div>
                                ...
                                <input type="date" wire:model="pembayaranForm.tanggal_pembayaran" 
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent" />
                            ...
                            <div>
                                ...
                                <select wire:model="pembayaranForm.bank_pengirim" 
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="pembayaranForm.nama_pengirim" 
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent" 
                        ...
                        <div class="flex justify-end mt-8">
                            <button type="button" wire:click="nextStep" 
                                class="bg-primary text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 transition duration-300">
                    ...
                    <div id="step2" class="{{ $formPage == 2 ? 'block' : 'hidden' }} bg-white rounded-lg shadow-lg p-8">
                        <div class="mb-8">
                            ...
                            <p class="text-gray-600">Upload bukti transfer pembayaran daftar ulang</p>
                        </div>
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-primary transition duration-300">
                            <div class="text-center">
                                ...
                                <p class="text-sm text-gray-600 mb-4">Format: JPG, PNG (Max. 2MB)</p>
                                <input type="file" wire:model="bukti_pembayaran" accept="image/*" class="hidden" id="bukti_pembayaran" />
                                <label for="bukti_pembayaran" class="bg-primary text-white px-6 py-2 rounded-lg cursor-pointer hover:bg-blue-700 transition duration-300">Pilih File</label>
                                <div id="bukti_pembayaran_preview" class="mt-4 {{ $bukti_pembayaran ? 'block' : 'hidden' }}">
                                    @if ($bukti_pembayaran)
                                        <img src="{{ $bukti_pembayaran->temporaryUrl() }}" class="mx-auto h-48 w-auto mb-4 rounded-lg">
                                        <p class="text-sm text-green-600">✓ File berhasil dipilih</p>
                                    @endif
                                </div>
                                @error('bukti_pembayaran') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    ...
                    <div id="step3" class="{{ $formPage == 3 ? 'block' : 'hidden' }} bg-white rounded-lg shadow-lg p-8">
                        ...
                        <div class="bg-gray-50 p-6 rounded-lg mb-6">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="font-medium">Rp {{ number_format((float)$pembayaranForm['nominal_pembayaran'] ?? 0, 0, ',', '.') }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500">Tanggal Transfer</p>
                                    <p class="font-medium">{{ $pembayaranForm['tanggal_pembayaran'] }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500">Bank Pengirim</p>
                                    <p class="font-medium">{{ $pembayaranForm['bank_pengirim'] }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500">Nama Pengirim</p>
                                    <p class="font-medium">{{ $pembayaranForm['nama_pengirim'] }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between mt-8">
                            ...
                            <button type="submit" 
                                ...
                                wire:target="submit"
                                class="bg-green-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-green-700 transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div wire:loading wire:target="submit" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white p-4 rounded-lg shadow-lg">
                    ...
                    <p class="mt-2 text-center text-gray-600">Sedang memproses...</p>
                </div>
            </div>
            ...
            <div class="fixed bottom-4 right-4 z-50">
                @if (session()->has('success'))
                    <div 
                        ...
                        class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg shadow-lg"
                        role="alert">
                        <div class="flex items-center">
    ...
    <script>
        document.addEventListener('livewire:load', function () {
            Livewire.on('show-success', event => {
                ...
                alertDiv.className = 'fixed bottom-4 right-4 z-50 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg shadow-lg';
                alertDiv.innerHTML = `
                    <div class="flex items-center">
                        <i class="fas fa-check-circle mr-2"></i>
                        <p>${message}</p>
            ...
            Livewire.on('show-error', event => {
                const message = event.message;
                const alertDiv = document.createElement('div');
                alertDiv.className = 'fixed bottom-4 right-4 z-50 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-lg';
                alertDiv.innerHTML = `
                    <div class="flex items-center">
                        <i class="fas fa-exclamation-circle mr-2"></i>
                        <p>${message}</p>
